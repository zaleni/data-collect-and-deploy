...
# Pick up the water bottle and place it into the box on the robot dog.
# Pick up the tissue and place it into the box on the robot dog.
# Pick up the McDonald bag and place it into the box on the robot dog.
# Pick up the chewing gum and place it into the box on the robot dog.   
...
# def post_process_action(action, prev_action=None, max_delta=0.1):
#     """
#     对动作进行后处理，限制范围并平滑变化。
...
#                 action[i] = prev_action[i] + np.sign(delta) * max_delta
...
def encode_obs(observation):
    obs={}
    obs['images'] = {}  
    print(observation['images']['head'].shape)
    obs['images']['cam_high'] = np.transpose (observation['images']['head'],(2,0,1))
    obs['images']['cam_left_wrist'] = np.transpose( observation['images']['left_wrist'],(2,0,1))

    # obs['images']['cam_high'] = np.array(observation['images']['head'])
    # obs['images']['cam_left_wrist'] = np.array(observation['images']['left_wrist'])

    obs['state']=observation['joint']
...
if __name__ == '__main__':
    ...
    pi0_inference=PI0Inference(args)
    ...
    while rclpy.ok():
        ...
        if t%args.chunk_size==0:
             print("infer observation:",observation['state'])
             action_buffer =pi0_inference.predict_action(observation)
             print(action_buffer)
             action_delta=observation['state']-action_buffer[0][:7]
             print("action delta:",action_delta)
        ...
        if action_buffer is not None:
            ...
            # current_action = post_process_action(current_action, prev_action=prev_action, max_delta=0.001)
