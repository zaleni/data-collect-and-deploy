...
class RobotStatus(metaclass=Metaclass_RobotStatus):
    ...
    def __init__(self, **kwargs):
        ...
        self.joint_cur = array.array('d', kwargs.get('joint_cur', []))
    ...
    def __eq__(self, other):
        ...
        if self.joint_vel != other.joint_vel:
            return False
        if self.joint_cur != other.joint_cur:
            return False
        return True
    ...
    def joint_cur(self, value):
        if isinstance(value, array.array):
            assert value.typecode == 'd', \
                "The 'joint_cur' array.array() must have the type code of 'd'"
        ...
        if __debug__:
            ...
            assert \
                ((isinstance(value, Sequence) or
                 ...
                 all(isinstance(v, float) for v in value) and
                 all(not (val < -1.7976931348623157e+308 or val > 1.7976931348623157e+308) or math.isinf(val) for val in value)), \
                "The 'joint_cur' field must be a set or sequence and each value of type 'float' and each double in [-.000000, .000000]"
        self._joint_cur = array.array('d', value)
