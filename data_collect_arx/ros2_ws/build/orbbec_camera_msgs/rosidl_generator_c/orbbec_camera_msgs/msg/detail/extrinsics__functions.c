  ...
  orbbec_camera_msgs__msg__Extrinsics__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    orbbec_camera_msgs__msg__Extrinsics * data =
      (orbbec_camera_msgs__msg__Extrinsics *)realloc(output->data, allocation_size);
    if (!data) {
      return false;
    }
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!orbbec_camera_msgs__msg__Extrinsics__init(&data[i])) {
        /* free currently allocated and return false */
        for (; i-- > output->capacity; ) {
          orbbec_camera_msgs__msg__Extrinsics__fini(&data[i]);
        }
        free(data);
        return false;
      }
