...
python -m sglang.launch_server --model-path Qwen/Qwen2.5-3B-Instruct --port 8013 --host 0.0.0.0
```

## 新开一个窗口，启动大脑VLM（启动一个就可以， 也可多个同时启动）
## 主机重启后要运行sudo mount /dev/sda1 /mnt
```bash
# snack 
...
# The lastest snack _5_14 去掉后加推盘子数据
export CUDA_VISIBLE_DEVICES=2
conda activate vllm
API_PORT=8001 llamafactory-cli api --model_name_or_path /home/pc3/deploy/vllm_model_ckpt/vllm_demo_snack_0514 --template paligemma --infer_backend huggingface --trust_remote_code true
...
export CUDA_VISIBLE_DEVICES=2
source /home/pc3/deploy/openpi/.venv/bin/activate
XLA_PYTHON_CLIENT_PREALLOCATE=false python /home/pc3/deploy/openpi/scripts_/deploy_server.py  --checkpoint_bucket /mnt/ckpts/magic/exp_order_food_0511/30000 --config_name pi0_bmlm_joint_snack_0511
