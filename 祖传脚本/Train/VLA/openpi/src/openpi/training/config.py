...
import openpi.policies.cfm_policy as cfm_policy
...
class LeRobotCFMDataConfig(DataConfigFactory):
    ...
    def create(self, assets_dirs: pathlib.Path, model_config: _model.BaseModelConfig) -> DataConfig:
        ...
        # replace the transforms below with your own.
        data_transforms = _transforms.Group(
            inputs=[cfm_policy.BmlmInputs(action_dim=model_config.action_dim, model_type=model_config.model_type)],
...
_CONFIGS = [
    ...
    TrainConfig(
        ...
        data=LeRobotCFMDataConfig(
            repo_id="datasets/SongLinTest-OrangePlacing",
            base_config=DataConfig(
        ...
        # Note that we load the pi0-FAST base model checkpoint here.
        weight_loader=weight_loaders.CheckpointWeightLoader("ckpts/pi0_fast_base/params"),
        num_train_steps=30_000,
        wandb_enabled=True,
    ...
    TrainConfig(
        ...
        data=LeRobotCFMDataConfig(
            repo_id = "datasets/SongLin-Remake",
            base_config=DataConfig(
                local_files_only=True,  # Set to True for local-only datasets.
                prompt_from_task=True,
                action_sequence_keys=("action", )
