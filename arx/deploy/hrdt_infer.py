...
def get_arguments():
    ...
    parser.add_argument('--model_port', type=int, default=9999,
...
def main():
    ...
    print("=" * 60)
    print("正在连接模型服务器...")
    model_client = ModelClient(host=args.model_host, port=args.model_port)
    ...
    # init_joint=[-0.09098148345947266,1.3521404266357422,2.393949508666992,-1.2983522415161133,-0.02574920654296875,-0.02384185791015625,0.004960060119628906]   
    ...
    while rclpy.ok():
        ...
        model_client.update_obs(observation)
        
        if t%args.chunk_size==0:
             action_buffer =model_client.predict_action(observation)
            #  print(action_buffer)
