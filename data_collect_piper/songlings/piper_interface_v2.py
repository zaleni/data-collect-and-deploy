...
from piper_sdk.protocol.protocol_v2 import C_PiperParserBase, C_PiperParserV2
from piper_sdk.piper_msgs.msg_v2 import *
from piper_sdk.kinematics import *
from piper_sdk.monitor import *
...
class C_PiperInterface_V2:
    ...
    def __init__(
        ...
        judge_flag=True,
        can_auto_init=True,
        saver: MessageSaver=None
    ) -> None:
        self.saver : MessageSaver = saver
        
        
        self.ArmStatus = ArmStatus
        self.ArmEndPose = ArmEndPose
        self.ArmJoint = ArmJoint
        self.ArmGripper = ArmGripper
        self.ArmMotorDriverInfoHighSpd = ArmMotorDriverInfoHighSpd
        self.ArmMotorDriverInfoLowSpd = ArmMotorDriverInfoLowSpd
        self.ArmMotorAngleLimitAndMaxVel = ArmMotorAngleLimitAndMaxVel
        self.CurrentEndVelAndAccParam = CurrentEndVelAndAccParam
        self.CrashProtectionLevelFeedback = CrashProtectionLevelFeedback
        self.GripperTeachingPendantParamFeedback = GripperTeachingPendantParamFeedback
        self.CurrentMotorMaxAccLimit = CurrentMotorMaxAccLimit
        self.ArmJointCtrl = ArmJointCtrl
        self.ArmGripperCtrl = ArmGripperCtrl
        self.ArmCtrlCode_151 = ArmCtrlCode_151
        self.AllCurrentMotorMaxAccLimit = AllCurrentMotorMaxAccLimit
        self.AllCurrentMotorAngleLimitMaxSpd = AllCurrentMotorAngleLimitMaxSpd
        self.ArmTimeStamp = ArmTimeStamp
    ...
    def GetFK(self):
        with self.__arm_joint_msgs_mtx:
            joint_states = [
                self.__arm_joint_msgs.joint_state.joint_1 / 1000,
    ...
    def UpdateArmEndPoseState(self, msg: PiperMessage):
        ...
        with self.__arm_end_pose_mtx:
            if msg.type_ == ArmMsgType.PiperMsgEndPoseFeedback_1:
                ...
                sth_has_changed = True
            self.__arm_end_pose.Hz = self.fps_counter.cal_average(
                self.fps_counter.get_fps("ArmEndPose_XY"),
            ...
            if sth_has_changed and self.saver is not None:
                self.saver.save_follower_endpose(self.__arm_end_pose)
            return self.__arm_end_pose
    ...
    def UpdateArmJointState(self, msg: PiperMessage):
        ...
        with self.__arm_joint_msgs_mtx:
            if msg.type_ == ArmMsgType.PiperMsgJointFeedBack_12:
                ...
                pass
            self.__arm_joint_msgs.Hz = self.fps_counter.cal_average(
                self.fps_counter.get_fps("ArmJoint_12"),
            ...
            if sth_has_changed and self.saver is not None:
                self.saver.save_follower_joint(self.__arm_joint_msgs)
    ...
    def UpdateArmGripperState(self, msg: PiperMessage):
        ...
        with self.__arm_gripper_msgs_mtx:
            ...
            # self.gripper_publisher.publish(self.__arm_gripper_msgs.to_joint_state())
            if sth_has_changed and self.saver is not None:
                self.saver.save_follower_gripper(self.__arm_gripper_msgs)
            return self.__arm_gripper_msgs
    ...
    def UpdateDriverInfoHighSpdFeedback(self, msg: PiperMessage):
        ...
        with self.__arm_motor_info_high_spd_mtx:
            if msg.type_ == ArmMsgType.PiperMsgHighSpdFeed_1:
                ...
                pass
            self.__arm_motor_info_high_spd.Hz = self.fps_counter.cal_average(
                self.fps_counter.get_fps("ArmMotorDriverInfoHighSpd_1"),
    ...
    def UpdateDriverInfoLowSpdFeedback(self, msg: PiperMessage):
        ...
        with self.__arm_motor_info_low_spd_mtx:
            if msg.type_ == ArmMsgType.PiperMsgLowSpdFeed_1:
                ...
                pass
            self.__arm_motor_info_low_spd.Hz = self.fps_counter.cal_average(
                self.fps_counter.get_fps("ArmMotorDriverInfoLowSpd_1"),
    ...
    def UpdateArmJointCtrl(self, msg: PiperMessage):
        ...
        with self.__arm_joint_ctrl_msgs_mtx:
            if msg.type_ == ArmMsgType.PiperMsgJointCtrl_12:
                ...
                pass
            self.__arm_joint_ctrl_msgs.Hz = self.fps_counter.cal_average(
                self.fps_counter.get_fps("ArmJointCtrl_12"),
            ...
            if sth_has_changed and self.saver is not None:
                # print(self.saver)
                # print(self.saver.save_leader_joint)
                # print(self.__arm_joint_ctrl_msgs)
                self.saver.save_leader_joint(self.__arm_joint_ctrl_msgs)
            return self.__arm_joint_ctrl_msgs
    ...
    def UpdateArmGripperCtrl(self, msg: PiperMessage):
        ...
        with self.__arm_gripper_ctrl_msgs_mtx:
            if msg.type_ == ArmMsgType.PiperMsgGripperCtrl:
                ...
                self.__arm_gripper_ctrl_msgs.Hz = self.fps_counter.get_fps(
                    "ArmGripperCtrl"
                )
            ...
            if sth_has_changed and self.saver is not None:
                self.saver.save_leader_gripper(self.__arm_gripper_ctrl_msgs)
            return self.__arm_gripper_ctrl_msgs
    ...
    def UpdateArmCtrlCode151(self, msg: PiperMessage):
        ...
        with self.__arm_ctrl_code_151_mtx:
            if msg.type_ == ArmMsgType.PiperMsgMotionCtrl_2:
                ...
                self.__arm_ctrl_code_151.Hz = self.fps_counter.get_fps(
                    "ArmCtrlCode_151"
                )
    ...
    def SetInstructionResponse(
        self, instruction_index: int, zero_config_success_flag: int
    ):
        """
        设置指令应答
    ...
    def GripperTeachingPendantParamConfig(
        self, teaching_range_per: int = 100, max_range_config: int = 70
    ):
        """
        ...
        gripper_teaching_pendant_param_config = ArmMsgGripperTeachingPendantParamConfig(
            teaching_range_per, max_range_config
        )
        msg = PiperMessage(
