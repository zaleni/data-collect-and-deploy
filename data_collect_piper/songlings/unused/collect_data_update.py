...
import numpy as np
from PyQt5.QtCore import QObject, pyqtProperty, pyqtSignal, pyqtSlot, QTimer, QThread, QUrl
from PyQt5.QtGui import QImage, QColor, QGuiApplication
from PyQt5.QtQml import QQmlApplicationEngine
from PyQt5.QtWidgets import QApplication
from PyQt5.QtQuick import QQuickImageProvider
from typing import Union, Literal, Final, get_args
import rospy
...
class SimpleSaver(MessageSaver):
    def __init__(self, base_dir):
        ...
        rospy.Subscriber('/ob_camera_01/color/image_raw', Image_msg, self.save_image_to(ImagePosFront, ImageTypeRGB, 'img_1_color_count'))
        rospy.Subscriber('/ob_camera_02/color/image_raw', Image_msg, self.save_image_to(ImagePosWrist, ImageTypeRGB, 'img_2_color_count'))
        rospy.Subscriber('/ob_camera_01/depth/image_raw', Image_msg, self.save_image_to(ImagePosFront, ImageTypeDepth, 'img_1_depth_count'))
        rospy.Subscriber('/ob_camera_02/depth/image_raw', Image_msg, self.save_image_to(ImagePosWrist, ImageTypeDepth, 'img_2_depth_count'))
    ...
    def start_saver(self):
        task_name = str(datetime.datetime.now().strftime("%m-%d@%H:%M:%S"))
...
class ViewModel(QObject):
    ...
    def x(self):
        ...
        attn = None
        if self._arm_state[0] < -16000:
            attn = '左'
        elif self._arm_state[0] > 0:
            attn = '右'
        if attn is not None:
            val = f'{val} {attn}'
        return val
    ...
    def y(self):
        ...
        attn = None
        if self._arm_state[1] < -83000:
            attn = '后'
        elif self._arm_state[1] > 0:
            attn = '前'
        if attn is not None:
            val = f'{val} {attn}'
        return val
    ...
    def z(self):
        ...
        attn = None
        if self._arm_state[2] < 330000:
            attn = '上'
        elif self._arm_state[2] > 380000:
            attn = '下'
        if attn is not None:
            val = f'{val} {attn}'
        return val
...
if __name__ == "__main__":
    ...
    rospy.Subscriber('/ob_camera_01/color/image_raw', Image_msg, camera_pipe.get_subscriber('front'))
    rospy.Subscriber('/ob_camera_02/color/image_raw', Image_msg, camera_pipe.get_subscriber('wrist'))
    
    saver = SimpleSaver('data/stage2')
    try:
